...
class Adam(Optimizer):
    ...
    def __init__(
        self, lr: float = 1e-3, beta_1: float = 0.9, beta_2: float = 0.999
    ) -> None:
        self.learning_rate = lr
    ...
    def applyGradients(
        ...
        if self.__m is None or len(self.__m) != len(weights) * sum(
            [len(i) for i in weights]
        ):
            self.__m = [0.0] * (len(weights) * sum([len(i) for i in weights]))
        if self.__v is None or len(self.__v) != len(weights) * sum(
            [len(i) for i in weights]
        ):
