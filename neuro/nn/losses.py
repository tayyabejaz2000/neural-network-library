...
from neuro.nn.activation import Softmax
...
class MeanSquaredError(Loss):
    def mean_squared_error(self, y_pred: tf.Variable, y_true: tf.Variable) -> tf.Variable:
        loss = tf.reduce_mean(tf.reduce_sum(
            tf.square(y_pred - y_true), axis=1), keepdims=True)
...
class BinaryCrossentropy(Loss):
    def binary_crossentropy_loss(self, y_pred: tf.Variable, y_true: tf.Variable) -> tf.Variable:
...
class CategoricalCrossentropy(Loss):
    def categorical_crossentropy_loss(self, y_pred: tf.Variable, y_true: tf.Variable) -> tf.Variable:
...
class SparseCategoricalCrossentropy(Loss):
    def sparse_categorical_crossentropy_loss(self, y_pred: tf.Variable, y_true: tf.Variable) -> tf.Variable:
        one_hot_labels = tf.one_hot(y_true, tf.unique(y_true)[0].shape[0])
        self._localGradients = y_pred - one_hot_labels
        return tf.reduce_mean(-tf.reduce_sum(one_hot_labels * tf.math.log(y_pred), axis=1))
